    ...
    <PublicShell>
      <nav className="border-b border-white/5 px-6 py-4 flex items-center justify-between sticky top-0 z-50 bg-[#0a0a0a]/90 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3">
          <Link href="/login" className="px-4 py-2 rounded-full border border-slate-200 text-sm text-slate-700 hover:border-brand-kinetic-orange hover:text-brand-kinetic-orange transition-colors">
            Entrar
          </Link>
      ...
      <main className="max-w-6xl mx-auto px-4 py-16 space-y-16">
        <section className="text-center space-y-5">
          <h1 className="text-4xl sm:text-5xl font-display font-bold tracking-tight text-slate-950">
          ...
          <div role="tablist" aria-label="Contenido de precios" className="mx-auto mt-8 inline-flex rounded-2xl border border-slate-200 bg-white p-1 shadow-sm">
            <button
              ...
              className={`min-h-[44px] rounded-xl px-5 text-sm font-bold transition-[background-color,color,box-shadow] duration-200 ${activeTab === "plans" ? "bg-brand-kinetic-orange text-black shadow-sm" : "text-slate-600 hover:bg-slate-50"}`}
            >
              Planes
            </button>
            <button
              ...
              className={`min-h-[44px] rounded-xl px-5 text-sm font-bold transition-[background-color,color,box-shadow] duration-200 ${activeTab === "addons" ? "bg-brand-kinetic-orange text-black shadow-sm" : "text-slate-600 hover:bg-slate-50"}`}
            >
              Add-ons
            </button>
          </div>
          ...
          {activeTab === "plans" && (
            <div className="flex items-center justify-center gap-3 mt-6">
              <span className={`text-sm ${!isAnnual ? "text-slate-950" : "text-brand-muted"}`}>Mensual</span>
              <button
                type="button"
              ...
              <span className={`text-sm ${isAnnual ? "text-slate-950" : "text-brand-muted"}`}>
        ...
        >
          {PLAN_ORDER.map((planKey) => {
            ...
            return (
              <article key={planKey} className="public-card relative rounded-3xl border border-slate-200 bg-white p-6 flex flex-col gap-5">
                {badge && (
                ...
                <div>
                  ...
                  <div className="flex items-end gap-1">
                    <span className="text-3xl font-display font-bold text-slate-950">Bs. {formatPrice(displayPrice)}</span>
                    <span className="text-brand-muted text-sm mb-1">/mes</span>
                  </div>
                  {isAnnual && <div className="text-brand-muted text-xs mt-0.5">Facturado anualmente</div>}
                  ...
                  {PLAN_FEATURES[planKey].map((feature) => (
                    <li key={feature.label} className="flex items-start gap-2 text-sm">
                      {feature.included ? (
                        ...
                        <X size={15} className="text-slate-300 flex-shrink-0 mt-0.5" aria-hidden="true" />
                      )}
                      <span className={feature.included ? "text-slate-700" : "text-slate-400"}>{feature.label}</span>
                    </li>
                  ))}
                </ul>
        ...
        >
          <div className="text-center">
            <h2 className="text-2xl font-display font-bold text-slate-950">Add-ons opcionales</h2>
            <p className="text-brand-muted mt-2 text-sm">Activa solo lo que tu operacion necesita.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {PUBLIC_ADDONS.map((addon) => (
              <article key={addon.key} className="public-card rounded-2xl border border-slate-200 bg-white p-5 space-y-2">
                <div className="flex justify-between items-start gap-2">
                  <h3 className="font-bold text-slate-950 text-sm">{addon.label}</h3>
        ...
        <section className="public-card rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-display font-bold text-slate-950">Comparacion rapida</h2>
            <p className="text-brand-muted mt-2 text-sm">Las funciones clave por etapa.</p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="text-center py-3 px-4 text-slate-950 font-bold">Basico</th>
                  <th className="text-center py-3 px-4 text-slate-950 font-bold">Crecer</th>
                  <th className="text-center py-3 px-4 text-slate-950 font-bold">Pro</th>
                  <th className="text-center py-3 px-4 text-slate-950 font-bold">Empresarial</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={String(row[0])} className="border-b border-slate-100">
                    ...
                    {[1, 2, 3, 4].map((col) => (
                      <td key={col} className="py-3 px-4 text-center">
                        {row[col] ? (
                          ...
                          <X size={15} className="text-slate-300 mx-auto" aria-hidden="true" />
                        )}
                      </td>
                    ))}
                  </tr>
        ...
        <section className="text-center space-y-4 py-8">
          <h2 className="text-2xl font-display font-bold text-slate-950">Listo para empezar?</h2>
          <p className="text-brand-muted">7 dias gratis, sin tarjeta de credito.</p>
          <Link
            href="/signup"
      ...
      <footer className="border-t border-slate-200 py-8 text-center text-xs text-brand-muted">
