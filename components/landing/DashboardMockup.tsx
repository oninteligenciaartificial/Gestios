    ...
    <>
      <style>{`
        @keyframes mockup-float {
          0%, 100% { transform: perspective(1000px) rotateX(3deg) rotateY(-6deg) translateY(0px); }
          50% { transform: perspective(1000px) rotateX(3deg) rotateY(-6deg) translateY(-4px); }
        }
        @keyframes bar-grow {
      ...
      <div className="hidden md:block relative w-full max-w-[520px] mx-auto">
        ...
        <div
          className="mockup-float relative rounded-2xl overflow-hidden border border-white/10 shadow-[0_0_60px_rgba(255,107,0,0.15)]"
        ...
        >
          ...
          <div className="flex items-center gap-2 px-4 py-3 bg-[#1a1c20] border-b border-white/5">
            ...
            {/* LIVE badge */}
            <div className="flex items-center gap-1.5 px-2 py-1 rounded-full bg-[#00af74]/15 border border-[#00af74]/20">
              <span className="w-1.5 h-1.5 rounded-full bg-[#00af74] animate-pulse" />
              <span className="text-[9px] text-[#00af74] font-medium">En vivo</span>
            </div>
          </div>
            ...
            <div className="w-36 bg-[#0c0e12] border-r border-white/5 flex flex-col py-3 shrink-0">
              ...
              <nav className="flex flex-col gap-0.5 px-2" aria-label="Dashboard navigation">
                {NAV_ITEMS.map((item) => (
                  ...
                  >
                    <span className="text-[12px]" aria-hidden="true">{item.icon}</span>
                    {item.label}
                  </div>
                ))}
              </nav>
            ...
            <div className="flex-1 overflow-hidden flex flex-col bg-[#111317]">
              ...
              <div className="flex-1 overflow-hidden p-3 space-y-3">
                ...
                <div className="grid grid-cols-5 gap-2">
                  {/* Mini bar chart */}
                  <div className="col-span-2 bg-[#1a1c20] rounded-lg p-2.5 border border-white/8 ring-1 ring-white/10">
                  ...
                  {/* Recent orders */}
                  <div className="col-span-3 bg-[#1a1c20] rounded-lg p-2.5 border border-white/8 ring-1 ring-white/10">
